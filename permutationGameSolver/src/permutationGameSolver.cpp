...
bool zoneContainer::openFile(const vectui& theZones, fstream& fs, bool writeMode, wstring& dbDirectory, wstring& fileType)
{
	...
	// convert wstring to string using a converter
	using convert_type = std::codecvt_utf8<wchar_t>;
	std::wstring_convert<convert_type, wchar_t> converter;
	std::string strFilename = converter.to_bytes(filename);

	// open file
	fs.open(strFilename, ios_base::binary | (writeMode ? ios_base::in | ios_base::out : ios_base::in));

	// if file does not exist than 
	if (!fs.is_open()) {
		if (writeMode) {
			fs.open(strFilename, ios_base::binary | ios_base::in | ios_base::out | ios_base::trunc);
		}
		return fs.is_open();
	// file exists
	} else {
...
bool zoneContainer::convertToNewestType(const vectui& theZones, fstream& fs, bool writeMode, wstring& filename)
{
	...
	if (myId != id) {
		...
		fs.close();
		fs.open(strFilename, ios_base::binary | ios_base::in | ios_base::out);
		saveToFile(fs);
		fs.close();
		fs.open(strFilename, ios_base::binary | (writeMode ? ios_base::in | ios_base::out : ios_base::in));
	}
...
bool seqContainer::loadFromFile(istream& is, bool loadSequences, bool loadCalculations)
{
	...
	is.read((char*) &totalCalculationTime		,	sizeof(uint64_t	));
...
bool seqContainer::saveToFile(ostream& os)
{
	...
	os.write((char*) &totalCalculationTime	,	sizeof(uint64_t	));
