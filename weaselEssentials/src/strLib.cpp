...
//-----------------------------------------------------------------------------
// Name: mystring() 
// Desc: constructor for ascii strings
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Name: mystring()
// Desc: constructor for wide char strings
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Name: c_strW()
// Desc: returns the wide char string
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Name: assign()
// Desc: set the string to a new ascii string
//-----------------------------------------------------------------------------
mystring & mystring::assign(const char *cStr)
{
	...
	size_t newReserved		= newLength * 2;
...
//-----------------------------------------------------------------------------
// Name: assign()
// Desc: set the string to a new wide char string
//-----------------------------------------------------------------------------
	...
	do {
		...
		default: 
			if (*curByte == decimalSeperator)  {
				decimalPlace = true;
				exponent	 = false;
			} else if (*curByte == columnSeparator || *curByte == '\n' || *curByte == '\r' || *curByte == '\0') {
				...
				if (exponent) {
					(*pData) *= std::pow(10, expIsNegativ ? -1*exponentialValue : 1);
				}
				...
				// end of file?
				if (*curByte == '\0') {
					if (curReadValue < numValues) {
						cout << "ERROR in function readAsciiData(): End of file reached before all values were read !" << endl;
						return false;
					}
					return true;
				}
