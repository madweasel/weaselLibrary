...
// ****************************************************************************************************
//
// CAUTION: This code is legacy and not maintained anymore. It is only provided for reference purpose.
//
// ****************************************************************************************************
	...
	hFontOutputBox		{ font }
{
	...
	outputStringBuf.str(L"");
	outputStream	= new wostream(&outputStringBuf);
...
bool miniMaxWinCalcDb::showControls(bool visible)
{
	...
	// initialize lists
	if (visible) {
		unsigned int	numLayers		= pMiniMax->game->getNumberOfLayers();
...
{
	layersToTest = {};
	for (unsigned int curLayer = 0; curLayer < pMiniMax->game->getNumberOfLayers(); curLayer++) {
...
void miniMaxWinCalcDb::buttonFuncCalcTestLayer(void* pUser)
{
	...
	if (curLayer >= pMiniMax->game->getNumberOfLayers()) return;
	layersToTest = {};
	layersToTest.push(curLayer);
	buttonFuncCalcTest();
}
...
void miniMaxWinCalcDb::threadProcTestLayer()
{
	...
	// calculate
	pMiniMax->threadManager.reset();
	pMiniMax->db.arrayInfos.init(pMiniMax->db.getNumLayers());
	pMiniMax->setOutputStream(*outputStream);
	pMiniMax->game->prepareCalculation();
	while (layersToTest.size()) {
		unsigned int curLayer = layersToTest.front();
		layersToTest.pop();
		if (pMiniMax->game->getNumberOfKnotsInLayer(curLayer) == 0) continue;
		pMiniMax->checker.testLayer(curLayer);
		pMiniMax->unloadDatabase();
...
void miniMaxWinCalcDb::updateOutputControls(void* pUser)
{
	...
	if (pMiniMaxWin->outputStringBuf.str().size()) {		// ... does not work properly - it always returns true
		...
		pMiniMaxWin->pMiniMax->monitor.getCurrentCalculatedLayer(curCalculatedLayers);
		...
		newOutputTextW << L"current action: " << pMiniMaxWin->pMiniMax->monitor.getCurrentActionStr();
	...
	while (pMiniMaxWin->pMiniMax->db.arrayInfos.anyArrayInfoToUpdate()) {
		pMiniMaxWin->updateListItemArray(pMiniMaxWin->pMiniMax->db.arrayInfos.getArrayInfoForUpdate());
	};
...
void miniMaxWinCalcDb::updateListItemLayer(unsigned int layerNumber)
{
	...
	vector<unsigned int> 	partnerLayers = pMiniMax->db.getPartnerLayers(layerNumber);

	// choose a color
	if (pMiniMax->db.isLayerCompleteAndInFile(layerNumber)) {
		if (pMiniMax->db.getNumberOfKnots(layerNumber) > 0) {
	...
	wssTmp.str(L""); wssTmp << (pMiniMax->db.isLayerCompleteAndInFile(layerNumber) ? L"Yes" : L"No");		hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 1), 0, wssTmp.str().c_str());
	wssTmp.str(L""); wssTmp << pMiniMax->db.getNumWonStates(layerNumber);									hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 2), 0, wssTmp.str().c_str());
	wssTmp.str(L""); wssTmp << pMiniMax->db.getNumLostStates(layerNumber);									hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 3), 0, wssTmp.str().c_str());
	wssTmp.str(L""); wssTmp << pMiniMax->db.getNumDrawnStates(layerNumber);									hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 4), 0, wssTmp.str().c_str());
	wssTmp.str(L""); wssTmp << pMiniMax->db.getNumInvalidStates(layerNumber);								hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 5), 0, wssTmp.str().c_str());
	wssTmp.str(L""); wssTmp << pMiniMax->db.getNumberOfKnots(layerNumber);									hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 6), 0, wssTmp.str().c_str());
	wssTmp.str(L""); wssTmp << pMiniMax->db.getLayerSizeInBytes(layerNumber);								hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 7), 0, wssTmp.str().c_str());
	wssTmp.str(L""); for (auto& partnerLayer : partnerLayers) wssTmp << partnerLayer << " ";				hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 8), 0, wssTmp.str().c_str());
	wssTmp.str(L""); wssTmp << pMiniMax->game->getOutputInformation(layerNumber);							hListViewLayer.setItemText(hListViewLayer.getItemIndex(layerNumber, 9), 0, wssTmp.str().c_str());
...
// Desc: called by MAIN-thread in pMiniMax->csOsPrint critical-section
//-----------------------------------------------------------------------------
void miniMaxWinCalcDb::updateListItemArray(database::arrayInfoChange infoChange)
{
	// locals
	wstringstream	wssTmp;
	...
	// array was created
		...
		{
		case database::arrayInfoStruct::arrayType::invalid:					wssTmp << "invalid";					break;
		case database::arrayInfoStruct::arrayType::countArray:				wssTmp << "count array";				break;
		case database::arrayInfoStruct::arrayType::plyInfos:				wssTmp << "plyInfo";					break;
		case database::arrayInfoStruct::arrayType::layerStats:				wssTmp << "short know value";			break;
		case database::arrayInfoStruct::arrayType::knotAlreadyCalculated:	wssTmp << "knots already calculated";	break;
