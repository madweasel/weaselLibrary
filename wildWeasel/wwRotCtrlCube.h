...
private:
	...
	void								createButton						(plainButton& myButton, vector3& position, vector3& rotation, vector3& scale, texture& tex, function<void(guiElemEvFol*, void*)> userFunc);
	void								rotate								(faceAreaIndex newFaceAreaState, unsigned int newRollState);
	void								calcStateAfter90DegRot				();
	faceAreaRollIndex					findStateMatrix						(matrix& toMatrix);
