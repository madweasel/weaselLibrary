...
//-----------------------------------------------------------------------------			
void wildWeasel::listView2D::insertColumn_plainButton2D(unsigned int columnIndex, wstring const& text, font2D* theFont, unsigned int widthColumn, float angle, float textSize, RECT& rcButton, buttonImageFiles& imageFiles, float autoWidthFraction)
{
	// locals
...
{
	int oldHeight = getRowHeight(rowIndex);
	auto myRow = getRow(rowIndex);
	myRow->sizeSetByUser	= height;
	myRow->curSize			= height;
	myRow->minSize			= height;

	for (unsigned int curCol = 0; curCol < getNumColumns(); curCol++) {
		auto& curElemCluster = *getItem(rowIndex, curCol)->getGuiElemPointer();
		for (auto& curItem : curElemCluster.items) {
			if (typeid(*curItem) == typeid(plainButton2D)) {	// TODO: <-- ... very very dirty !!!
				auto myButton2D = static_cast<plainButton2D*>(curItem);
				RECT rc;
				rc.left		= 0;
				rc.top		= 0;
				rc.bottom	= myRow->sizeSetByUser;
				rc.right	= getColumn(curCol)->sizeSetByUser;
				myButton2D->setTargetRect(rc);
				myButton2D->setPosition(0, -oldHeight, true);
			}
		}
	}
	updateSledgeWidth();
...
//-----------------------------------------------------------------------------
// Name: setRowTextColor()
// Desc: 
//-----------------------------------------------------------------------------
void wildWeasel::listView2D::setRowTextColor(unsigned int rowIndex, color newColor)
{
	// locals
	unsigned int	startIndex		= 0;
	unsigned int	numItemsToMark	= getNumColumns();
	...
	// remember and set new color for each item in row
	processItems(startIndex, numItemsToMark, indexStep, [&](list<item>::iterator& curItem) {
		curItem->subItems->setTextColor(newColor);
	});
}
