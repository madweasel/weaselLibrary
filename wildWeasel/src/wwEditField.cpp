...
// Desc: 
//-----------------------------------------------------------------------------
void wildWeasel::editField2D::insertString(wstringbuf& streamBuffer)
{
	// directly fetch the string from the buffer
	const wstring& newOutputTextW = streamBuffer.str();

	// any input?
	if (newOutputTextW.empty()) return;

	// insert the string
	insertString(newOutputTextW.c_str());
...
void wildWeasel::editField2D::eventGotFocus(guiElemEvFol* elem, void* pUser)
{
	...
	// set cursor (... muss noch optimiert werden, da die linien liste unn�tig durchlaufen wird !!!)
