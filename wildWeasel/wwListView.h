...
public:									
	...
	void								insertColumn_plainButton2D			(unsigned int columnIndex, wstring& text, font2D* theFont, unsigned int widthColumn, float angle, float textSize, RECT& rcButton, buttonImageFiles& imageFiles, float autoSizeFactor = 0.0f);
	...
	// MAYBE: not implemented yet (the problem is that sprites cannot be clipped diagonally)
